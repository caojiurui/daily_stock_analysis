...
def _is_retryable_chip_fetch_error(exc: Exception) -> bool:
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        for attempt in range(1, 3):
            try:
                ...
                logger.info(f"[API调用] ak.stock_cyq_em(symbol={stock_code}) 获取筹码分布... (attempt {attempt}/2)")
                import time as _time
                api_start = _time.time()
                ...
                last_error = e
                logger.warning(f"[API错误] 获取 {stock_code} 筹码分布失败 (attempt {attempt}/2): {e}")
                if attempt >= 2 or not _is_retryable_chip_fetch_error(e):
                    break
                time.sleep(1.0)
